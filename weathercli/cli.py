...
from weathercli.config import init_config_file, set_api_key, get_config, get_config_path
...
def read_user_cli_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=3,
        help=(
            "The number of forecast periods (3h) to query. Should be between 1 and 40. "
            "Invalid inputs will be replaced by the default of 3."
        ),
    )
    parser.add_argument(
        "--version", action="store_true", help="Print weather CLI version."
    )
    parser.add_argument(
        "--api-key", type=str, help="Creates config.ini and stores API key"
    )
...
def display_forecast_info(weather_data: Dict, imperial: bool = False) -> None:
    ...
    for forecast in weather_data["list"]:
        ...
        units = "F" if imperial else "C"
        print(
            f"{temperature:>4.1f}°{units} (feels like {temperature_feel:>4.1f}°{units})"
        )
...
def cli() -> None:
    ...
    if user_args.forecast:
        query_url = build_historical_query(
            user_args.city, user_args.imperial, cnt=user_args.number
        )
